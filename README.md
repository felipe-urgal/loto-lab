...
A arquitetura está em transição incremental. Várias features já possuem controllers finos e use cases injetados; `src/api/app.ts` ainda concentra concursos, análises, Generator 2.0, geração compatível e parte dos game batches. A #61 rastreia a remoção desse ownership restante sem rewrite.
...
Use cases já extraídos incluem análise básica/avançada, geração compatível, conferência, backtest, Strategy Lab, catálogo de estratégias/backtests, operações, apostas reais e status de dados. A composição concreta fica progressivamente em `server.ts`.

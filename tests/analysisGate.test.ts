...
import { LotoLabApiServices } from "../src/api/services.js";
...
function fakePool(readContests: () => Contest[]): Pool {
  return {
    query: async () => {
      const rows = readContests().map((contest) => ({
        lottery: contest.lottery,
        contest_number: contest.number,
        draw_date: contest.date,
...
test("advanced analysis shares in-flight work and serves cache while the expensive gate is busy", async () => {
  ...
  const services = new LotoLabApiServices(fakePool(() => contests));
  ...
  const firstRequest = services.analyzeAdvanced("mega-sena");
  const secondRequest = services.analyzeAdvanced("mega-sena");
  ...
  try {
    const cached = await services.analyzeAdvanced("mega-sena");
    ...
    await assert.rejects(
      () => services.analyzeAdvanced("mega-sena"),
      (error: unknown) => {
  ...
  const refreshed = await services.analyzeAdvanced("mega-sena");

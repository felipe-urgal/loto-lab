...
test("Jobs exposes Strategy Lab v2 options and evidence-aware language", async () => {
  const [html, boundary, jobs, presentation] = await Promise.all([
  ...
  assert.match(presentation, /Melhor no período:/);
  assert.match(presentation, /insufficient-resolution/);
  assert.doesNotMatch(presentation, /Vencedor:/);
  assert.match(presentation, /function statusClass\(status: string\)/);

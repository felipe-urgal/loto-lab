...
function load(lottery, force = false) {
  ...
  if (!cache.has(lottery)) {
    cache.set(lottery, api(`/analysis/${lottery}`).catch((error) => {
...
function tabsMarkup() {
  return `<nav class="a2-tabs" aria-label="Modos de análise">${TABS.map((tab) => `
    <button type="button" data-a2-tab="${tab}" class="${activeTab === tab ? "is-active" : ""}" aria-selected="${activeTab === tab}">${TAB_LABELS[tab]}</button>
  `).join("")}</nav>`;
}
...
function shellMarkup(data) {
  ...
  return `<div class="a2-shell">
    ...
    ${tabsMarkup()}
    <section id="a2-view"></section>
    <aside class="a2-detail" id="a2-detail" hidden aria-label="Detalhe da dezena"></aside>
  </div>`;
}

function ballList(numbers, tier) {
  return numbers.map((value) => `<button class="a2-ball ${tierClass(tier)}" type="button" data-a2-number="${value}">${number(value)}</button>`).join("");
}
  ...
  return `<div class="a2-stack">
    ...
    <section><div class="section-head"><div><h2>Ranking auditável</h2><p>Movimento usa a posição de 10 concursos atrás; robustez perturba todos os pesos em ±10%.</p></div></div>
      <div class="panel table-wrap"><table class="a2-table"><thead><tr><th>#</th><th>Dezena</th><th>Grupo</th><th>Score</th><th>Mov. 10</th><th>Tendência</th><th>Robustez</th><th>Atraso</th></tr></thead><tbody>${dynamics.map((item) => `
        <tr data-a2-number="${item.number}" tabindex="0"><td><strong>${item.rank}</strong></td><td><strong>${number(item.number)}</strong></td><td><span class="a2-tier-chip ${tierClass(item.tier)}">${TIER_LABELS[item.tier]}</span></td><td>${decimal(item.score)}</td><td>${movementBadge(item.movements.ten)}</td><td>${trendCopy(item.trend)}</td><td>${percent(item.weightRobustness.tierStability)}</td><td>${item.delay.current} concurso(s)</td></tr>`).join("")}</tbody></table></div>
    </section>
  </div>`;
}
...
function dynamicsView(data) {
  ...
  const delayed = [...advanced.ranking.dynamics.items].sort((a, b) => b.delay.percentile - a.delay.percentile || b.delay.current - a.delay.current).slice(0, 12);
  return `<div class="a2-stack">
    ...
    <section><div class="section-head"><div><h2>Ciclo descritivo</h2><p>Tempo necessário para o conjunto inteiro de dezenas aparecer ao menos uma vez; não é previsão de saída.</p></div></div><article class="panel a2-cycle"><div>${metricCard("Ciclo atual", cycles.currentLength, `${cycles.seen} dezenas já vistas`)}</div><div class="a2-cycle-copy"><strong>${cycles.missing.length ? `${cycles.missing.length} ainda não vistas` : "ciclo recém-completado"}</strong><div class="a2-ball-cloud">${cycles.missing.map((value) => `<button type="button" data-a2-number="${value}" class="a2-ball">${number(value)}</button>`).join("")}</div><small>Média histórica de duração: ${cycles.historicalLength ? decimal(cycles.historicalLength.mean) : "—"} concursos · ${cycles.completedCount} ciclo(s) completos.</small></div></article></section>
    <section><div class="section-head"><div><h2>Atraso em contexto</h2><p>Percentil alto significa que o atraso atual foi pouco frequente no histórico daquela própria dezena; não significa maior chance no próximo sorteio.</p></div></div><div class="panel a2-delay-list">${delayed.map((item) => `<button type="button" data-a2-number="${item.number}"><strong>${number(item.number)}</strong><span>${item.delay.current} concurso(s)</span><em>percentil ${Math.round(item.delay.percentile * 100)}</em></button>`).join("")}</div></section>
...
function robustnessRows(data) {
  return [...data.advanced.ranking.dynamics.items]
    ...
    .map((item) => `<tr data-a2-number="${item.number}" tabindex="0"><td><strong>${number(item.number)}</strong></td><td>#${item.rank}</td><td>${TIER_LABELS[item.tier]}</td><td>${percent(item.weightRobustness.tierStability)}</td><td>${percent(item.weightRobustness.strongShare)}</td><td>#${item.weightRobustness.rankRange[0]}–#${item.weightRobustness.rankRange[1]}</td></tr>`).join("");
}
...
function numberDetailMarkup(item) {
  ...
  return `<div class="a2-detail-head"><div><span>Dezena ${number(item.number)}</span><h2>#${item.rank} · ${decimal(item.score)}</h2><div><span class="a2-tier-chip ${tierClass(item.tier)}">${TIER_LABELS[item.tier]}</span><span>${trendCopy(item.trend)}</span></div></div><button type="button" data-a2-detail-close aria-label="Fechar detalhe">×</button></div>
    <div class="a2-detail-grid">${metricCard("Movimento 10", item.movements.ten === null ? "—" : `${item.movements.ten > 0 ? "+" : ""}${item.movements.ten}`, "posições")}${metricCard("Robustez", percent(item.weightRobustness.tierStability), `rank #${item.weightRobustness.rankRange[0]}–#${item.weightRobustness.rankRange[1]}`)}${metricCard("Atraso", item.delay.current, `percentil ${Math.round(item.delay.percentile * 100)}`)}${metricCard("Sequência atual", item.streak, "concursos consecutivos presente")}</div>
...
function openNumberDetail(value) {
  ...
  detail.hidden = false;
  detail.querySelector("[data-a2-detail-close]")?.addEventListener("click", () => { detail.hidden = true; });
}
...
function bindShellInteractions() {
  root.querySelectorAll("[data-a2-tab]").forEach((button) => button.addEventListener("click", () => {
    activeTab = button.dataset.a2Tab;
    ...
    root.querySelector("#a2-detail")?.setAttribute("hidden", "");
    renderTab();
  }));
}

async function renderAnalysisV2(force = false) {
  if (!root || currentView() !== "analysis") return;
  const lottery = currentLottery();
  const token = ++renderToken;
  try {
    const data = await load(lottery, force);
    ...
    root.innerHTML = `<div class="error-state"><span class="error-code">ANALYSIS_V2</span><strong>Não foi possível carregar Análises 2.0</strong><p>${escapeHtml(error.message || "Erro inesperado")}</p><button class="button" type="button" data-a2-retry>Tentar novamente</button></div>`;
    root.querySelector("[data-a2-retry]")?.addEventListener("click", () => { void renderAnalysisV2(true); });
  }
}
...
document.querySelector("#refresh-view")?.addEventListener("click", () => {
  if (currentView() === "analysis") void renderAnalysisV2(true);
});

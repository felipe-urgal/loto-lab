...
function agendaBadge(item: NavigationItem): string {
  return item.key === "agenda"
    ? '<span class="agenda-nav-badge" data-agenda-nav-badge hidden></span>'
    : "";
}

function desktopItem(item: NavigationItem): string {
...
function menuItem(item: NavigationItem): string {
...
function closeMore(restoreFocus = false): void {
  const button = document.querySelector<HTMLButtonElement>("[data-nav-more]");
  const panel = document.querySelector<HTMLElement>("[data-nav-more-menu]");
...
if (nav) {
  ...
  const moreButton = nav.querySelector<HTMLButtonElement>("[data-nav-more]");
  const morePanel = nav.querySelector<HTMLElement>("[data-nav-more-menu]");
  ...
  document.addEventListener("click", (event) => {
    if (!(event.target instanceof Node) || !nav.contains(event.target)) closeMore();
  });
